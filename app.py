...
def analyze_pair_smc_ml(pair, market_type):
    ...
    try:
        if pair not in SMC_ML_PAIRS:
            # Allow both slash and non-slash forms
            normalized = pair.replace("USDT", "/USDT") if "USDT" in pair and "/" not in pair else pair
            if normalized not in SMC_ML_PAIRS and pair not in ("BTCUSDT", "ETHUSDT", "SOLUSDT", "EUR/USD", "GBP/USD", "USD/JPY", "XAU/USD"):
                return
