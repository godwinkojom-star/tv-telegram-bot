...
import news_engine
from alcr_strategy import ALCRAnalyzer, ALCRConfig, ALCR_PAIRS
import smc_ml_research
from smc_ml_research import SMCMLConfig, analyze_1h, SMC_ML_PAIRS
import pandas as pd
...
# 2.39.2 - ALCR Telegram restored: public channel signals + public result/expiry
#          cards. Private chat stays silent for ALCR. News filter kept.
# 2.39.1 - ALCR research-trade expiry aligned to 12h (same as V2/V3).
# 2.39.0 - ALCR dashboard connection: qualifying SIGNAL rows write to the
#          main signals table with strategy_version=ALCR, get sequential IDs,
#          open_trade monitoring, optional AutoBot. Public channel Telegram
#          restored for ALCR (no private chat). Research log unchanged.
#
# 2.38.0 - Added ALCR v1 research strategy as an isolated third strategy.
#          ALCR uses the shared get_candles()/OHLC cache, writes research to
#          alcr_research_log; qualifying SIGNAL rows also enter the dashboard
#          signals table (no Telegram). AutoBot optional.
#          pipeline, and has its own persistent ON/OFF switch. News Engine
#          High Impact alerts now use the public channel callback instead of
#          the private Telegram chat.
# 2.37.0 - New news engine: replaced deleted news_engine.py with a full
#          SmartFS Forex Calendar Service integration. Reuses news_filter.py
#          (High Impact only, 30 min before / 15 min after). Blocks NEW
#          signals only via emit_signal_from_result. Own heartbeat on
#          news_engine_state, upcoming + protection-ended graphic cards
#          to private Telegram, activity_log events. Fail-safe blocks
#          when calendar data is unavailable. V2/V3 strategy logic,
#          scan cadence, and signal/result/expiry behavior unchanged.
VERSION = "2.39.2"
...
_STRATEGY_DEFAULTS = {"V2": True, "V3": False, "ALCR": True}
_strategy_enabled_cache = {"V2": True, "V3": False, "ALCR": True}
...
def _normalize_strategy_version(strategy_version):
    ...
    if s in ("V3", "3"):
        return "V3"
    if s in ("ALCR", "ALCR_V1", "ALCR_V1.0", "ALCR_V1.0.0"):
        return "ALCR"
    return s
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT strategy_v2_enabled, strategy_v3_enabled, strategy_alcr_enabled FROM bot_status WHERE id = 1"
                )
                row = cur.fetchone()
        conn.close()
        if row is not None:
            v2, v3, alcr = row[0], row[1], row[2]
            if v2 is not None:
                flags["V2"] = bool(v2)
            if v3 is not None:
                flags["V3"] = bool(v3)
            if alcr is not None:
                flags["ALCR"] = bool(alcr)
    except Exception as e:
        log_error(f"Strategy flags read failed (fail-safe V2=ON V3=OFF ALCR=ON): {e}")
        flags = dict(_STRATEGY_DEFAULTS)
    _strategy_enabled_cache = flags
    _strategy_cache_loaded = True
    log_info(
        "Strategy flags loaded: V2=%s V3=%s ALCR=%s"
        % ("ON" if flags.get("V2") else "OFF", "ON" if flags.get("V3") else "OFF", "ON" if flags.get("ALCR") else "OFF")
    )
    return flags
...
# POST-ENTRY trade outcome tracking window (NOT Gold V2 setup/MSS expiry).
# Once a signal is entered, monitor TP/SL for up to 12 hours using real
# candle/price evaluation in monitor_trades. Only if no terminal outcome
# occurs within this window is the trade marked EXPIRED.
TRADE_TRACKING_MAX_SECONDS = 12 * 3600
SIGNAL_EXPIRY_SECONDS = {
    "1m": TRADE_TRACKING_MAX_SECONDS,   # Gold V2 entry TF (post-entry)
    "5m": TRADE_TRACKING_MAX_SECONDS,   # V3 trigger TF (post-entry)
    "15m": TRADE_TRACKING_MAX_SECONDS,
    "1h": TRADE_TRACKING_MAX_SECONDS,
}
...
def _strategy_tag_from_trade(trade):
    """Normalized [V2]/[V3] for public/private result messaging."""
    sv = trade.get("strategy_version") if isinstance(trade, dict) else None
...
def _icon_link(draw, cx, cy, r, color):
    c = color if (isinstance(color, tuple) and len(color) >= 4) else tuple(list(color)[:3]) + (255,)
    draw.ellipse([cx - r, cy - r, cx + r, cy + r], outline=c, width=2)


def _draw_instrument_badge(draw, cx, cy, r, pair, accent=None):
    """Local vector instrument badge — no external assets."""
    p = str(pair or "").upper().replace(" ", "")
    fill, ring, label = (40, 40, 40, 255), (180, 180, 180, 255), "?"
    if "BTC" in p:
        fill, ring, label = (247, 147, 26, 255), (255, 200, 80, 255), "B"
    elif "ETH" in p:
        fill, ring, label = (98, 126, 234, 255), (160, 180, 255, 255), "E"
    elif "SOL" in p:
        fill, ring, label = (0, 200, 180, 255), (120, 255, 230, 255), "S"
    elif "XAU" in p or "GOLD" in p:
        fill, ring, label = (212, 175, 55, 255), (255, 220, 120, 255), "Au"
    elif "EUR" in p or "GBP" in p or "JPY" in p or "/" in str(pair or ""):
        fill, ring, label = (50, 90, 140, 255), (140, 180, 220, 255), "FX"
    draw.ellipse([cx - r, cy - r, cx + r, cy + r], fill=fill, outline=ring, width=2)
    draw.text((cx, cy), label, font=_card_font(18 if len(label) < 2 else 14, bold=True),
...
def generate_result_card(trade, outcome, final_tp_label=None):
    ...
    # Deep pure black background — no green/red wash
    img = Image.new("RGBA", (W, H), (0, 0, 0, 255))
    for y in range(H):
        g = int(10 * (y / H))
        ImageDraw.Draw(img).line([(0, y), (W, y)], fill=(g, g, g, 255))

    # Neutral gray candles (not accent-colored wash)
    try:
        _draw_candlestick_bg(img, 640, 160, 960, 520, (90, 90, 90), rng, count=7)
    except Exception:
        pass
    ...
    # ---- Header ----
    _icon_badge(draw, 90, 95, 40, (70, 70, 70))
    draw.text((90, 95), "SFX", font=_card_font(22, bold=True), fill=(245, 245, 245, 255), anchor="mm")
    ...
    row_y = 350
    _draw_instrument_badge(draw, 90, row_y, 26, trade.get("pair"))
    draw.text((135, row_y - 22), trade["pair"], font=_card_font(38, bold=True), fill=(245, 245, 245, 255))

    row_y = 415
    _icon_arrow(draw, 90, row_y, 14, (200, 200, 200), up=(trade["direction"] == "BUY"))
    dir_font = _card_font(24, bold=True)
    draw.text((120, row_y - 15), trade["direction"], font=dir_font, fill=(245, 245, 245, 255))
...
def send_result_card(trade, outcome, final_tp_label=None):
    # Result cards always go to the PUBLIC channel (not private).
    try:
...
def send_public_photo(image_bytes, caption=None):
    return send_telegram_photo(TELEGRAM_PUBLIC_CHANNEL_ID, image_bytes, caption=caption)
...
def send_private_photo(image_bytes, caption=None):
    """Send a photo to the private Telegram chat (news alerts, etc.)."""
    return send_telegram_photo(TELEGRAM_PRIVATE_USER_ID, image_bytes, caption=caption)
...
def init_db():
    ...
    try:
            ...
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    ...
                    SET strategy_v2_enabled = COALESCE(strategy_v2_enabled, TRUE),
                        ...
                        strategy_alcr_enabled = COALESCE(strategy_alcr_enabled, TRUE)
                    WHERE id = 1
                """)
                try:
                    cur.execute("""
        ...
        conn.close()
        # ALCR research-only storage. This is deliberately separate from live signals.
        try:
            conn = get_db_connection()
            with conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS alcr_research_log (
                    ...
                    """)
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_alcr_research_pair_created ON alcr_research_log (pair, created_at DESC)")
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_alcr_research_status_created ON alcr_research_log (status, created_at DESC)")
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_alcr_research_strategy_created ON alcr_research_log (strategy_version, created_at DESC)")
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS alcr_research_trades (
                            id BIGSERIAL PRIMARY KEY,
                            research_log_id BIGINT REFERENCES alcr_research_log(id) ON DELETE SET NULL,
                            pair TEXT NOT NULL,
                            market_type TEXT NOT NULL,
                    ...
                    """)
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_alcr_research_trades_pair_status ON alcr_research_trades (pair, status)")
                    cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_alcr_one_active_pair ON alcr_research_trades (pair) WHERE status = 'ACTIVE'")
                    cur.execute("ALTER TABLE alcr_research_log ENABLE ROW LEVEL SECURITY")
                    cur.execute("GRANT SELECT ON alcr_research_log TO authenticated")
                    cur.execute("""
                        DO $$ BEGIN
                            CREATE POLICY alcr_research_select_authenticated ON alcr_research_log
                            FOR SELECT TO authenticated USING (true);
                        EXCEPTION WHEN duplicate_object THEN NULL; END $$;
                    """)
                    cur.execute("ALTER TABLE alcr_research_trades ENABLE ROW LEVEL SECURITY")
                    cur.execute("GRANT SELECT ON alcr_research_trades TO authenticated")
                    cur.execute("""
                        DO $$ BEGIN
                            CREATE POLICY alcr_research_trades_select_authenticated ON alcr_research_trades
                            FOR SELECT TO authenticated USING (true);
                        EXCEPTION WHEN duplicate_object THEN NULL; END $$;
                    """)
                    cur.execute("""
                        DO $$ BEGIN
                            IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime')
                               AND NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'alcr_research_log')
                            THEN ALTER PUBLICATION supabase_realtime ADD TABLE public.alcr_research_log; END IF;
                            IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime')
                               AND NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'alcr_research_trades')
                            THEN ALTER PUBLICATION supabase_realtime ADD TABLE public.alcr_research_trades; END IF;
                        EXCEPTION WHEN undefined_object THEN NULL; END $$;
                    """)
            conn.close()
        except Exception as e:
            log_error(f"ALCR research table setup failed: {e}")
        
        # --- SMC ML Research tables (research-only, no live signals) ---
        try:
            conn = get_db_connection()
            with conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS smc_ml_research_log (
                            id BIGSERIAL PRIMARY KEY,
                            pair TEXT NOT NULL,
                            market_type TEXT NOT NULL,
                            strategy_version TEXT NOT NULL DEFAULT 'SMC_ML_V1.0',
                            structure_type TEXT,
                            direction TEXT,
                            entry DOUBLE PRECISION,
                            sl DOUBLE PRECISION,
                            ...
                            rr_to_tp1 DOUBLE PRECISION,
                            rr_to_tp2 DOUBLE PRECISION,
                            retest_probability DOUBLE PRECISION,
                            entry_mode TEXT,
                    ...
                    """)
                    cur.execute("""
                        CREATE TABLE IF NOT EXISTS smc_ml_research_trades (
                            id BIGSERIAL PRIMARY KEY,
                            research_log_id BIGINT,
                            pair TEXT NOT NULL,
                            market_type TEXT NOT NULL,
                            direction TEXT NOT NULL,
                            entry DOUBLE PRECISION NOT NULL,
                    ...
                    """)
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_smc_ml_log_pair ON smc_ml_research_log (pair)")
                    cur.execute("CREATE INDEX IF NOT EXISTS idx_smc_ml_trades_pair_status ON smc_ml_research_trades (pair, status)")
                    cur.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_smc_ml_one_active_pair ON smc_ml_research_trades (pair) WHERE status = 'ACTIVE'")
                    cur.execute("ALTER TABLE smc_ml_research_log ENABLE ROW LEVEL SECURITY")
                    cur.execute("GRANT SELECT ON smc_ml_research_log TO authenticated")
                    cur.execute("""
                        DO $$ BEGIN
                            CREATE POLICY smc_ml_research_select_authenticated ON smc_ml_research_log
                            FOR SELECT TO authenticated USING (true);
                        EXCEPTION WHEN duplicate_object THEN NULL; END $$;
                    """)
                    cur.execute("ALTER TABLE smc_ml_research_trades ENABLE ROW LEVEL SECURITY")
                    cur.execute("GRANT SELECT ON smc_ml_research_trades TO authenticated")
                    cur.execute("""
                        DO $$ BEGIN
                            CREATE POLICY smc_ml_research_trades_select_authenticated ON smc_ml_research_trades
                            FOR SELECT TO authenticated USING (true);
                        EXCEPTION WHEN duplicate_object THEN NULL; END $$;
                    """)
            conn.close()
            log_info("SMC ML research tables confirmed/created.")
        except Exception as e:
            log_error(f"SMC ML research table setup failed: {e}")


        log_info("Database ready: 'signals', 'bot_status', 'pair_scan_status', 'activity_log', and 'alcr_research_log' tables confirmed/created.")
    except Exception as e:
...
def db_insert_signal(pair, market_type, timeframe, direction, result, signal_id, strategy_version="V2", strategy_variant=None):
    """
    Insert a signal row. strategy_version mandatory: "V2", "V3", or "ALCR".
    ...
    ALCR rows are dashboard-only (no Telegram).
    """
    sv = _normalize_strategy_version(strategy_version)
    if sv not in ("V2", "V3", "ALCR"):
...
def close_trade(trade_id, trade, outcome, keep_tracking=False, trigger_info=None):
    ...
    _sv_tag = _strategy_tag_from_trade(trade)
    _sv_disp = f" {_sv_tag}" if _sv_tag else ""
    extra_note = "\n\n(Still tracking to see if TP2/TP3 are also reached...)" if keep_tracking else ""
    _is_alcr = (_normalize_strategy_version(trade.get("strategy_version")) == "ALCR")

    # ALCR: public channel only (result cards). No private chat messages.
    # V2/V3: keep private text alerts as before.
    if not _is_alcr:
        send_private_message(
            f"{result_emoji} *Signal #{signal_id}{_sv_disp} Result: {outcome}*\n\n"
...
def expire_trade(trade_id, trade):
    ...
    expiry_seconds = SIGNAL_EXPIRY_SECONDS.get(trade["timeframe"], TRADE_TRACKING_MAX_SECONDS)
    ...
    signal_id = trade.get("signal_id", "N/A")
    _sv_tag = _strategy_tag_from_trade(trade)
    _sv_disp = (" " + _sv_tag) if _sv_tag else ""
    _is_alcr = (_normalize_strategy_version(trade.get("strategy_version")) == "ALCR")

    # Private expiry text: V2/V3 only. Public channel expiry: all strategies including ALCR.
    if not _is_alcr:
        send_private_message(
            "⏳ *Signal #" + str(signal_id) + _sv_disp + " Expired*\n\n"
            + str(trade["pair"]) + " | " + str(trade["timeframe"]) + " | " + str(trade["direction"]) + "\n"
            + ("No TP1 or Stop Loss hit within %.0fh - closed without a result." % hours)
        )

    direction = trade.get("direction", "")
    dir_icon = "📈" if str(direction).upper() == "BUY" else "📉"
    public_expired = (
        "⏳ *SIGNAL EXPIRED*\n\n"
        + "🆔 Signal #" + str(signal_id) + _sv_disp + "\n\n"
        + "💹 " + str(trade["pair"]) + "\n"
        + "⏰ " + str(trade["timeframe"]) + "\n"
        + dir_icon + " " + str(direction) + "\n\n"
        + "🟠 *TIMEOUT — NO RESULT*\n\n"
        + ("No TP1 or Stop Loss was hit within %.0fh. " % hours)
        + "The signal has been closed without a result.\n\n"
        + "💡 Not every setup completes. Stay disciplined and wait for the next opportunity."
    )
    send_public_signal(public_expired)

    send_push_to_all(
        "signal_result_alerts",
        "SmartFX Result ⏱️",
        str(trade["pair"]) + " " + str(trade["direction"]) + " — EXPIRED" + _sv_disp,
...
# ==========================================================
# STRATEGY SCAN DIAGNOSTICS (observability only — no gate changes)
# ==========================================================
...
V2_GOLD_GATE_ORDER = [
    ...
    "1M MSS",
    "Setup Invalidation",
    "Late Entry / Anti-Chase",
    "Structural SL / Risk",
    "Dynamic TP1",
    "TP1 Freshness",
    "Signal / Management completion",
]

# Map analyze_gold_v2 reason codes → first failing gate name
V2_REASON_TO_GATE = {
    "15m_trend_filter_not_aligned": "15M Trend Filter",
    "volatility_not_ready": "Market Condition",
    "volatility_or_movement_too_low": "Market Condition",
    "healthy_volatility": "Market Condition",
    "spread_cost_gate_failed": "Spread Cost",
    "no_meaningful_15m_liquidity": "15M Liquidity",
    "no_valid_5m_sweep_reclaim": "5M Sweep/Reclaim",
    "1m_mss_not_confirmed_within_window": "1M MSS",
    "sweep_setup_invalidated_before_entry": "Setup Invalidation",
    "entry_not_chased": "Late Entry / Anti-Chase",
    "move_already_extended": "Late Entry / Anti-Chase",
    "stop_loss_too_large_or_invalid": "Structural SL / Risk",
    "no_valid_dynamic_tp1": "Dynamic TP1",
    "tp1_already_reached_before_entry": "TP1 Freshness",
...
V3_GATE_ORDER = [
    ...
    "5M ATR",
    "Anti-Chase",
    "Structural Targets / RR",
]


def _gate_results_until_fail(order, failed_gate, skip_gates=None):
    """PASS before fail, FAIL at fail, NOT_REACHED after. skip_gates marked NOT_REACHED always."""
    skip = set(skip_gates or [])
    rows = []
    failed = False
    for g in order:
        if g in skip:
            rows.append({"gate": g, "result": "NOT_REACHED"})
            continue
        if failed:
            rows.append({"gate": g, "result": "NOT_REACHED"})
        elif failed_gate and g == failed_gate:
            rows.append({"gate": g, "result": "FAIL"})
            failed = True
        else:
            rows.append({"gate": g, "result": "PASS"})
    return rows


def _v2_gate_results_from_raw(raw):
    """Build gate_results from existing analyze_gold_v2 output (no logic change)."""
    if not raw or not isinstance(raw, dict):
        return [], None, None
    reason = raw.get("reason")
    status = raw.get("status")
    if status == "SIGNAL" or reason == "all_required_gates_passed":
        return [{"gate": g, "result": "PASS"} for g in V2_GOLD_GATE_ORDER
                if g != "Spread Cost"], None, reason or "all_required_gates_passed"
    failed = V2_REASON_TO_GATE.get(reason)
    if failed is None and reason:
        # Unknown reason: mark last generic fail as Late Entry if contains chase, else Structural
        failed = "15M Trend Filter"
        for key, gate in V2_REASON_TO_GATE.items():
            if key and reason and key in str(reason):
                failed = gate
                break
    # Spread Cost only applicable when that gate runs; otherwise mark NOT_REACHED when fail is earlier
    skip = []
    if failed and failed != "Spread Cost":
        # If we never hit spread gate in this path, still show it as NOT_REACHED when fail is before it
        idx_fail = V2_GOLD_GATE_ORDER.index(failed) if failed in V2_GOLD_GATE_ORDER else 0
        idx_spread = V2_GOLD_GATE_ORDER.index("Spread Cost")
        if idx_fail < idx_spread:
            skip.append("Spread Cost")
        elif idx_fail > idx_spread:
            # spread was applicable and passed (not the fail reason)
            pass
    gates = _gate_results_until_fail(V2_GOLD_GATE_ORDER, failed, skip_gates=skip)
    return gates, failed, reason


def db_insert_strategy_scan_log(
    pair, market_type, strategy_version, timeframe, candidate_direction,
    candidate_detected, outcome, failed_gate=None, reason=None,
    gate_results=None, details=None,
):
    """Insert one observability row. Never raises into scanner path."""
    if not DATABASE_URL:
        return
    try:
        sv = _normalize_strategy_version(strategy_version) or strategy_version
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO strategy_scan_log (
                        pair, market_type, strategy_version, timeframe, scanned_at,
                        candidate_direction, candidate_detected, outcome,
                        failed_gate, reason, gate_results, details
                    ) VALUES (
                        %s, %s, %s, %s, %s,
                        %s, %s, %s,
                        %s, %s, %s::jsonb, %s::jsonb
                    ...
                    """,
                    (
                        pair, market_type, sv, timeframe, datetime.utcnow(),
                        candidate_direction, bool(candidate_detected), outcome,
                        failed_gate, reason,
        ...
        log_error("strategy_scan_log insert failed for %s %s: %s" % (pair, strategy_version, e))


def _log_scan_activity(strategy_label, pair, outcome, candidate_direction=None, failed_gate=None, strategy_version=None):
    """One meaningful activity_log line per strategy evaluation."""
    try:
        if outcome == "SIGNAL":
            msg = "✅ %s %s — signal qualified" % (strategy_label, pair)
        elif outcome == "NO_DATA":
            msg = "⚠️ %s %s — no market data" % (strategy_label, pair)
        elif outcome == "ERROR":
            msg = "⚠️ %s %s — analysis error" % (strategy_label, pair)
        elif candidate_direction and failed_gate:
            msg = "❌ %s %s — rejected at %s" % (strategy_label, pair, failed_gate)
        elif candidate_direction:
            msg = "🔎 %s %s — directional candidate: %s (no signal)" % (
                strategy_label, pair, candidate_direction)
        else:
            msg = "🔄 %s %s — no directional candidate" % (strategy_label, pair)
        db_log_activity("scanner_diagnostic", msg, strategy_version=strategy_version)
    except Exception as e:
        log_error("scan activity log failed: %s" % e)


def _diagnose_v3_pipeline(candles_4h, candles_1h, candles_15m, candles_5m):
    """
    Mirror existing V3 sequential stages for observability only.
    Uses the same smc_analysis_v3 functions; does not change gate conditions.
    Returns (gate_results, failed_gate, reason, bias).
    """
    gates = []
    bias = None
    try:
        bias, bias_source = smc_analysis_v3.get_4h_bias(candles_4h)
    except Exception as e:
        return [{"gate": g, "result": "NOT_REACHED"} for g in V3_GATE_ORDER], "4H Bias", "bias_exception:%s" % e, None
    if bias is None:
        gates = _gate_results_until_fail(V3_GATE_ORDER, "4H Bias")
        return gates, "4H Bias", "no_4h_bias", None
    gates.append({"gate": "4H Bias", "result": "PASS"})

    setup = smc_analysis_v3.detect_1h_setup(candles_1h, bias)
    if not setup or not setup.get("valid"):
        gates += _gate_results_until_fail(
            [g for g in V3_GATE_ORDER if g != "4H Bias"], "1H Structure / Setup")
        # rebuild with 4H PASS
        return (
            [{"gate": "4H Bias", "result": "PASS"}]
            + _gate_results_until_fail(
                [g for g in V3_GATE_ORDER if g != "4H Bias"], "1H Structure / Setup"),
            "1H Structure / Setup",
            "1h_setup_invalid",
            bias,
    ...
    zone = smc_analysis_v3.get_15m_entry_zone(candles_15m, bias)
    if zone is None:
        return (
            _gate_results_until_fail(V3_GATE_ORDER, "15M Entry Zone"),
            "15M Entry Zone", "no_15m_zone", bias,
        )

    if not smc_analysis_v3.price_has_returned_to_zone(
        candles_15m, zone["zone_low"], zone["zone_high"]
    ):
        return (
            _gate_results_until_fail(V3_GATE_ORDER, "15M Zone Return"),
            "15M Zone Return", "price_not_returned_to_zone", bias,
        )

    trigger = smc_analysis_v3.detect_5m_trigger(
        candles_5m, bias, zone["zone_low"], zone["zone_high"]
    )
    if not trigger or not trigger.get("valid"):
        return (
            _gate_results_until_fail(V3_GATE_ORDER, "5M Trigger"),
            "5M Trigger", "5m_trigger_invalid", bias,
        )

    atr_5m = smc_analysis_v3.calculate_atr(candles_5m)
    if atr_5m is None:
        return (
            _gate_results_until_fail(V3_GATE_ORDER, "5M ATR"),
            "5M ATR", "atr_unavailable", bias,
        )

    entry = candles_5m[-1]["close"]
    if smc_analysis_v3.anti_chase_check(
        entry, bias, zone["zone_low"], zone["zone_high"], atr_5m
    ):
        return (
            _gate_results_until_fail(V3_GATE_ORDER, "Anti-Chase"),
            "Anti-Chase", "anti_chase_failed", bias,
        )

    targets = smc_analysis_v3.calculate_structural_targets(
        entry, bias, zone["zone_low"], zone["zone_high"], candles_1h, atr_5m
    )
    if targets is None:
        return (
            _gate_results_until_fail(V3_GATE_ORDER, "Structural Targets / RR"),
            "Structural Targets / RR", "targets_invalid", bias,
        )

    return (
        [{"gate": g, "result": "PASS"} for g in V3_GATE_ORDER],
        None,
...
# ==========================================================
# ALCR RESEARCH STRATEGY ADAPTER (V1.0)
# Research only: never calls emit_signal_from_result(), open_trade(),
# AutoBot, Telegram, or live signals.
# ==========================================================

ALCR_SIGNAL_TIMEFRAME = "5m"
ALCR_FOREX_PAIRS = ["EUR/USD", "GBP/USD", "USD/JPY"]
ALCR_CRYPTO_PAIRS = ["BTCUSDT", "ETHUSDT", "SOLUSDT"]
ALCR_CRYPTO_MODULE_PAIRS = {
    "BTCUSDT": "BTC/USDT",
    "ETHUSDT": "ETH/USDT",
    "SOLUSDT": "SOL/USDT",
}
ALCR_ANALYZER = ALCRAnalyzer(ALCRConfig())
...
def _alcr_market_type(pair):
    return "crypto" if pair in ALCR_CRYPTO_PAIRS else "forex"


def _alcr_pair_name(pair):
    return ALCR_CRYPTO_MODULE_PAIRS.get(pair, pair)


def _alcr_fetch_bundle(pair, market_type):
    if market_type == "forex":
        tfs = ("1h", "15m", "5m")
    else:
        tfs = ("4h", "1h", "15m", "5m")
    ...
    for tf in tfs:
        try:
            ...
            log_error(f"ALCR candle fetch failed for {pair} {tf}: {e}")
            return None
        if not candles:
            return None
        frames[tf] = _candles_to_dataframe(candles)
        if market_type == "forex" and tf != "5m":
            time.sleep(FOREX_INNER_CALL_DELAY_SECONDS)
    return frames


def _alcr_correlation_group(pair):
    if pair in ("EUR/USD", "GBP/USD"):
        return "USD_FX_EUROPE"
    if pair in ALCR_CRYPTO_PAIRS:
        return "CRYPTO_L1"
    if pair == "USD/JPY":
        return "USD_JPY"
    return None
...
def _alcr_get_active_trade(pair):
    if not DATABASE_URL:
        return None
    try:
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id, research_log_id, pair, market_type, setup_type, direction,
                           entry, sl, tp1, tp2, score, opened_at, expires_at, status,
                           mfe, mae, session
                    FROM alcr_research_trades
                    WHERE pair = %s AND status = 'ACTIVE'
        ...
        if not row:
            return None
        cols = [
            "id","research_log_id","pair","market_type","setup_type","direction",
            "entry","sl","tp1","tp2","score","opened_at","expires_at","status",
            "mfe","mae","session"
        ]
        return dict(zip(cols, row))
    except Exception as e:
        log_error(f"ALCR active trade read failed for {pair}: {e}")
        return None


def _alcr_last_completed_at(pair):
    if not DATABASE_URL:
        return None
    try:
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT completed_at FROM alcr_research_trades
                    WHERE pair = %s AND status = 'COMPLETED'
        ...
        log_error(f"ALCR completed-trade read failed for {pair}: {e}")
        return None


def db_insert_alcr_research(result, pair, market_type):
    if not DATABASE_URL or not result:
        return None
    try:
        bd = result.get("score_breakdown") or {}
        details = {
            "candidate_detected": result.get("candidate_detected"),
            "candidate_direction": result.get("candidate_direction"),
            "reasons": result.get("reasons", []),
            "hard_failures": result.get("hard_failures", []),
            "all_candidates": result.get("all_candidates", []),
            "metadata": result.get("metadata", {}),
            "context_bias": result.get("context_bias"),
        }
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO alcr_research_log (
                        pair, market_type, strategy_version, setup_type, market_state,
                        volatility_state, timeframe, direction, score,
                        environment_score, location_score, setup_score, momentum_score,
                        structure_score, risk_score, target_score, entry, sl, tp1, tp2,
                        risk_reward, candidate_detected, status, outcome, reason,
                        failed_gate, detected_at, triggered_at, expires_at, session,
                        correlation_group, analysis_details
                    ) VALUES (
                        %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
                        %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb
                    ) RETURNING id
                """, (
                    pair, market_type, result.get("strategy_version", "ALCR_V1.0"),
                    result.get("setup_type"), result.get("regime"), result.get("volatility_state"),
                    result.get("timeframe", ALCR_SIGNAL_TIMEFRAME), result.get("candidate_direction"),
                    result.get("score"), bd.get("environment"), bd.get("location"),
                    bd.get("setup_quality"), bd.get("momentum"), bd.get("structure"),
                    bd.get("risk_quality"), bd.get("target_quality"), result.get("entry"),
                    result.get("stop_loss"), result.get("tp1"), result.get("tp2"),
                    result.get("rr_to_tp1"), bool(result.get("candidate_detected")),
                    result.get("status", "NO_SIGNAL"), result.get("outcome"), result.get("reason"),
                    result.get("failed_gate"), result.get("setup_detected_at"),
                    result.get("trigger_at"), result.get("expires_at"),
                    result.get("session"), _alcr_correlation_group(pair), json.dumps(details),
                ))
                row = cur.fetchone()
        conn.close()
        return row[0] if row else None
    except Exception as e:
        log_error(f"ALCR research log insert failed for {pair}: {e}")
        return None


def _alcr_open_research_trade(result, pair, market_type, research_log_id):
    if not DATABASE_URL or result.get("status") != "SIGNAL":
        return
    if _alcr_get_active_trade(pair):
        return
    try:
        # Same 12h post-entry window as V2/V3 (TRADE_TRACKING_MAX_SECONDS)
        expires_at = datetime.utcnow() + timedelta(seconds=TRADE_TRACKING_MAX_SECONDS)
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO alcr_research_trades (
                        research_log_id, pair, market_type, setup_type, direction,
                        entry, sl, tp1, tp2, score, opened_at, expires_at, session
                    ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW(),%s,%s)
                    ON CONFLICT DO NOTHING
                """, (
                    research_log_id, pair, market_type, result.get("setup_type") or "UNKNOWN",
                    result.get("candidate_direction"), result.get("entry"), result.get("stop_loss"),
                    result.get("tp1"), result.get("tp2"), result.get("score"),
                    expires_at, result.get("session")
                ))
        conn.close()
    except Exception as e:
        log_error(f"ALCR research trade open failed for {pair}: {e}")


def monitor_alcr_research_trades():
    """Research-only lifecycle monitor. Never calls live trade/AutoBot code."""
    if not DATABASE_URL:
        return
    try:
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id, research_log_id, pair, market_type, direction, entry, sl, tp1,
                           tp2, opened_at, expires_at, mfe, mae
                    FROM alcr_research_trades WHERE status = 'ACTIVE'
                """)
                rows = cur.fetchall()
        conn.close()
    except Exception as e:
        log_error(f"ALCR research trade fetch failed: {e}")
        return

    for row in rows:
        try:
            (trade_id, log_id, pair, market_type, direction, entry, sl, tp1, tp2,
             opened_at, expires_at, mfe, mae) = row
            price = get_current_price(pair, market_type)
            risk = abs(float(entry) - float(sl))
            if risk <= 0:
                continue
            if direction == "BUY":
                mfe_now = max(0.0, (price - entry) / risk)
                mae_now = max(0.0, (entry - price) / risk)
                hit_sl = price <= sl
                hit_tp = price >= tp1
            else:
                mfe_now = max(0.0, (entry - price) / risk)
                mae_now = max(0.0, (price - entry) / risk)
                hit_sl = price >= sl
                hit_tp = price <= tp1
            new_mfe = max(float(mfe or 0), mfe_now)
            new_mae = max(float(mae or 0), mae_now)
            now = datetime.utcnow()
            outcome = None
            if hit_sl:
                outcome = "LOSS"
            ...
            if outcome:
                if outcome == "WIN":
                    r_mult = round((tp1 - entry) / risk if direction == "BUY" else (entry - tp1) / risk, 4)
                elif outcome == "LOSS":
                    r_mult = -1.0
                else:
                    r_mult = 0.0
                ...
                with conn:
                    with conn.cursor() as cur:
                        cur.execute("""
                            UPDATE alcr_research_trades
                            SET status='COMPLETED', outcome=%s, exit_price=%s,
                                completed_at=NOW(), mfe=%s, mae=%s, r_multiple=%s
                            WHERE id=%s AND status='ACTIVE'
                        """, (outcome, price, new_mfe, new_mae, r_mult, trade_id))
                        if log_id:
                            cur.execute("""
                                UPDATE alcr_research_log
                                SET status='COMPLETED', outcome=%s, completed_at=NOW(),
                                    mfe=%s, mae=%s, r_multiple=%s
                                WHERE id=%s
                            """, (outcome, new_mfe, new_mae, r_mult, log_id))
                conn.close()
            else:
                conn = get_db_connection()
                with conn:
                    with conn.cursor() as cur:
                        cur.execute("UPDATE alcr_research_trades SET mfe=%s, mae=%s WHERE id=%s", (new_mfe, new_mae, trade_id))
                conn.close()
        except Exception as e:
            log_error(f"ALCR research trade monitor failed: {e}")


def analyze_pair_alcr(pair, market_type):
    """Run ALCR analysis. Research log always; SIGNAL rows go to dashboard (no Telegram)."""
    if not is_strategy_enabled("ALCR"):
        return
    if pair not in ALCR_PAIRS and pair not in ALCR_CRYPTO_PAIRS:
        return
    try:
        bundle = _alcr_fetch_bundle(pair, market_type)
        if not bundle:
            result = {
                "status": "NO_DATA", "outcome": "NO_DATA", "reason": "insufficient_candles",
                "candidate_detected": False, "candidate_direction": None,
                "strategy_version": "ALCR_V1.0", "timeframe": ALCR_SIGNAL_TIMEFRAME,
            }
            db_insert_alcr_research(result, pair, market_type)
            return
        module_pair = _alcr_pair_name(pair)
        active_trade = _alcr_get_active_trade(pair)
        last_completed_at = _alcr_last_completed_at(pair)
        result = ALCR_ANALYZER.analyze(
            module_pair, bundle,
            active_trade=bool(active_trade),
            last_completed_at=last_completed_at,
            now=bundle["5m"].index[-1],
        )
        result["session"] = None
        try:
            from alcr_strategy import classify_session
            result["session"] = classify_session(bundle["5m"].index[-1])
        except Exception:
            pass
        research_log_id = db_insert_alcr_research(result, pair, market_type)
        _alcr_open_research_trade(result, pair, market_type, research_log_id)
        log_info(
            "ALCR %s %s — %s score=%s dir=%s setup=%s reason=%s"
            % (pair, market_type, result.get("status"), result.get("score"),
               result.get("candidate_direction"), result.get("setup_type"), result.get("reason"))
        )
        # Dashboard path: only true SIGNAL rows (score gate already inside ALCR)
        try:
            st = str(result.get("status") or "").upper()
            if st == "SIGNAL" or result.get("is_signal"):
                emit_alcr_to_dashboard(pair, market_type, result)
        except Exception as e:
            log_error("ALCR dashboard emit failed for %s (research path OK): %s" % (pair, e))
    except Exception as e:
        log_error(f"ALCR engine error for {pair} (isolated; V2/V3 unaffected): {e}")
        try:
            db_insert_alcr_research({
                "status": "ERROR", "outcome": "ERROR", "reason": str(e),
                "candidate_detected": False, "candidate_direction": None,
                "strategy_version": "ALCR_V1.0", "timeframe": ALCR_SIGNAL_TIMEFRAME,
            }, pair, market_type)
        except Exception:
...
def run_alcr_analysis_for_pair(pair, market_type):
    try:
        analyze_pair_alcr(pair, market_type)
    except Exception as e:
        log_error(f"ALCR isolated wrapper error for {pair}: {e}")


# ==========================================================
# SMC ML RESEARCH ENGINE (research-only, Supabase only)
# ==========================================================

def _smc_ml_candles_to_df(candles):
    """Convert list-of-dict candles from get_candles() into a DataFrame."""
    if not candles:
        return None
    rows = []
    for c in candles:
        rows.append({
            ...
            "time": c.get("time") or c.get("datetime"),
        })
    df = pd.DataFrame(rows)
    if "time" in df.columns:
        df["time"] = pd.to_datetime(df["time"], utc=True, errors="coerce")
...
def db_insert_smc_ml_research(result, pair, market_type):
    if not DATABASE_URL:
        return None
    try:
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO smc_ml_research_log (
                        pair, market_type, strategy_version, structure_type, direction,
                        entry, sl, tp1, tp2, risk, rr_to_tp1, rr_to_tp2,
                        retest_probability, entry_mode, timeframe, status, reason,
                        detected_at, expires_at, analysis_details
                    ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                    RETURNING id
                """, (
                    pair, market_type,
                    result.get("strategy_version", "SMC_ML_V1.0"),
                    result.get("structure_type"),
                    result.get("direction"),
                    result.get("entry"),
                    result.get("stop_loss") or result.get("sl"),
                    result.get("tp1"),
                    result.get("tp2"),
                    result.get("risk"),
                    result.get("rr_to_tp1"),
                    result.get("rr_to_tp2"),
                    result.get("retest_probability"),
                    ...
                    json.dumps(result.get("analysis_details") or {}),
                ))
                row = cur.fetchone()
        conn.close()
        return row[0] if row else None
    except Exception as e:
        log_error(f"SMC ML research log insert failed for {pair}: {e}")
        return None


def _smc_ml_get_active_trade(pair):
    if not DATABASE_URL:
        return None
    try:
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id FROM smc_ml_research_trades
                    WHERE pair = %s AND status = 'ACTIVE' LIMIT 1
                """, (pair,))
                row = cur.fetchone()
        conn.close()
        return row[0] if row else None
...
def _smc_ml_open_research_trade(result, pair, market_type, research_log_id):
    if not DATABASE_URL or result.get("status") != "SIGNAL":
        return
    if _smc_ml_get_active_trade(pair):
        return
    try:
        expires_at = datetime.utcnow() + timedelta(hours=100)
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO smc_ml_research_trades (
                        research_log_id, pair, market_type, direction,
                        entry, sl, tp1, tp2, opened_at, expires_at
                    ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,NOW(),%s)
                    ON CONFLICT DO NOTHING
                """, (
                    research_log_id, pair, market_type,
                    result.get("direction"),
                    result.get("entry"),
                    result.get("stop_loss") or result.get("sl"),
                    result.get("tp1"),
        ...
        log_error(f"SMC ML research trade open failed for {pair}: {e}")


def monitor_smc_ml_research_trades():
    """Research-only lifecycle monitor. Never touches live trading or Telegram."""
    if not DATABASE_URL:
        return
    try:
        conn = get_db_connection()
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT id, research_log_id, pair, market_type, direction,
                           entry, sl, tp1, tp2, opened_at, expires_at, mfe, mae
                    FROM smc_ml_research_trades WHERE status = 'ACTIVE'
                """)
                rows = cur.fetchall()
        conn.close()
    except Exception as e:
        log_error(f"SMC ML research trade fetch failed: {e}")
        return

    for row in rows:
        try:
            (trade_id, log_id, pair, market_type, direction,
             entry, sl, tp1, tp2, opened_at, expires_at, mfe, mae) = row

            high = low = close = None
            try:
                candles = get_candles(pair, "1h", market_type, use_cache=True)
                if candles:
                    last = candles[-1]
                ...
                log_error(f"SMC ML monitor candle fetch failed for {pair}: {e}")
            ...
            if high is not None and low is not None and close is not None and exit_reason is None:
                if direction == "BUY":
                    if low <= sl:
                        ...
                        # leave open; optional BE move can be added later
                        pass
                else:
                    if high >= sl:
                        exit_price, exit_reason = sl, "sl"
            ...
            with conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        UPDATE smc_ml_research_trades
                        SET status='CLOSED', exit_price=%s, exit_reason=%s,
                            completed_at=NOW(), r_multiple=%s
                        WHERE id=%s
                    """, (exit_price, exit_reason, r_mult, trade_id))
                    if log_id:
                        cur.execute("""
                            UPDATE smc_ml_research_log
                            SET outcome=%s, completed_at=NOW(), r_multiple=%s
                            WHERE id=%s
                        """, (exit_reason.upper(), r_mult, log_id))
            conn.close()
            log_info(f"SMC ML research closed {pair} {direction} → {exit_reason} R={r_mult:.2f}")
        except Exception as e:
            log_error(f"SMC ML monitor error on trade: {e}")


def analyze_pair_smc_ml(pair, market_type):
    """Research-only entry point. Uses shared get_candles() for 1H data."""
    try:
        log_info(f"SMC ML scan start {pair} {market_type}")

        allowed = ("BTCUSDT", "ETHUSDT", "SOLUSDT", "EUR/USD", "GBP/USD", "USD/JPY", "XAU/USD")
        if pair not in SMC_ML_PAIRS and pair not in allowed:
            log_info(f"SMC ML {pair} — skip (pair not in allow-list)")
            return

        candles = get_candles(pair, "1h", market_type, use_cache=True)
        n_candles = len(candles) if candles else 0
        log_info(f"SMC ML {pair} — got {n_candles} x 1H candles")

        df_1h = _smc_ml_candles_to_df(candles)
        if df_1h is None or len(df_1h) < 80:
            log_info(f"SMC ML {pair} — skip (need >=80 bars, have {0 if df_1h is None else len(df_1h)})")
            return

        cfg = SMCMLConfig(entry_mode="adaptive", timeout_bars=100)
        signal = analyze_1h(df_1h, pair, market_type, cfg)

        if signal is None:
            log_info(f"SMC ML {pair} — NO_SETUP (no BOS/CHoCH on latest bar)")
            return

        result = signal.as_dict()
        result["stop_loss"] = result.get("stop_loss") or result.get("sl")

        log_id = db_insert_smc_ml_research(result, pair, market_type)
        log_info(f"SMC ML {pair} — db log_id={log_id} status={result.get('status')}")

        if result.get("status") == "SIGNAL" and log_id:
            _smc_ml_open_research_trade(result, pair, market_type, log_id)
            log_info(
                f"SMC ML {pair} {market_type} — SIGNAL {result.get('direction')} "
                f"{result.get('structure_type')} retest_prob={result.get('retest_probability', 0):.2f}"
            )
        else:
            log_info(f"SMC ML {pair} — {result.get('status')} ({result.get('reason')})")

    except Exception as e:
        log_error(f"SMC ML engine error for {pair} (isolated): {e}")
        try:
            db_insert_smc_ml_research({
                "status": "ERROR",
                "reason": str(e),
                "strategy_version": "SMC_ML_V1.0",
                "timeframe": "1h",
            }, pair, market_type)
        except Exception:
            pass
...
def run_smc_ml_analysis_for_pair(pair, market_type):
    try:
        analyze_pair_smc_ml(pair, market_type)
    except Exception as e:
        log_error(f"SMC ML isolated wrapper error for {pair}: {e}")
        import traceback
...
def run_v2_gold_analysis(pair, market_type):
    ...
    df15, df5, df1 = bundle
    bias = None
    # Single shared V2Config (defaults match smc_analysis_v2) for bias + analyze
    gold_cfg = smc_analysis_v2.V2Config()
    try:
        trend = smc_analysis_v2.determine_15m_bias(df15, gold_cfg)
        bias = trend.get("bias") if isinstance(trend, dict) and trend.get("bias") in ("BUY", "SELL") else None
    except Exception as e:
        log_error("Gold V2 bias error for %s: %s" % (pair, e))
    try:
        raw = smc_analysis_v2.analyze_gold_v2(df15, df5, df1, config=gold_cfg)
    except Exception as e:
...
def analyze_pair_v2(pair, market_type):
    ...
    Observability: writes strategy_scan_log + one activity event (no gate changes).
    """
    ...
    try:
        bundle = fetch_v2_gold_candle_bundle(pair, market_type)
        if bundle is None:
            db_insert_strategy_scan_log(
                pair, market_type, "V2", V2_SIGNAL_TIMEFRAME, None, False,
                "NO_DATA", reason="insufficient_candles",
            )
            _log_scan_activity("V2 Gold", pair, "NO_DATA", strategy_version="V2")
            db_update_pair_status(
                pair, market_type, None, "WATCHING", None, None, strategy_version="V2"
            )
            return
        df15, df5, df1 = bundle
        gold_cfg = smc_analysis_v2.V2Config()
        ...
        try:
            ...
            log_error("Gold V2 analyze_gold_v2 error for %s: %s" % (pair, e))
            db_insert_strategy_scan_log(
                pair, market_type, "V2", V2_SIGNAL_TIMEFRAME, bias, bool(bias),
                "ERROR", reason=str(e),
            )
            _log_scan_activity("V2 Gold", pair, "ERROR", candidate_direction=bias, strategy_version="V2")
            db_update_pair_status(
                pair, market_type, bias if bias in ("BUY", "SELL") else None,
                "WATCHING", None, None, strategy_version="V2",
            )
            return

        gate_results, failed_gate, reason = _v2_gate_results_from_raw(raw)
        mapped = map_gold_v2_result_to_signal(raw)
        cand_dir = bias
        if mapped and mapped.get("direction") in ("BUY", "SELL"):
            cand_dir = mapped["direction"]
        candidate_detected = cand_dir in ("BUY", "SELL")

        if mapped is None:
            db_insert_strategy_scan_log(
                pair, market_type, "V2", V2_SIGNAL_TIMEFRAME, cand_dir, candidate_detected,
                "NO_SIGNAL", failed_gate=failed_gate, reason=reason,
                gate_results=gate_results,
                details={"raw_status": (raw or {}).get("status")},
            )
            _log_scan_activity(
                "V2 Gold", pair, "NO_SIGNAL",
                candidate_direction=cand_dir, failed_gate=failed_gate, strategy_version="V2",
            )
        ...
        db_insert_strategy_scan_log(
            pair, market_type, "V2", V2_SIGNAL_TIMEFRAME, cand_dir, True,
            "SIGNAL", reason=reason or "all_required_gates_passed",
            gate_results=gate_results,
        )
        _log_scan_activity("V2 Gold", pair, "SIGNAL", candidate_direction=cand_dir, strategy_version="V2")
        ...
        try:
            ...
            _log_scan_activity("V2 Gold", pair, "ERROR", strategy_version="V2")
        except Exception:
            pass
...
def normalize_strategy_signal(pair, market_type, timeframe, result, strategy_version):
    ...
    # ALCR uses candidate_direction / stop_loss; V2/V3 use direction / sl
    raw_dir = result.get("direction")
    if raw_dir is None:
        raw_dir = result.get("candidate_direction")
    direction = _parse_direction_buy_sell(raw_dir)
    if direction is None:
        return None, "unrecognized direction=%r" % (raw_dir,)
    entry = _as_float(result.get("entry"))
    sl = _as_float(result.get("sl"))
    if sl is None:
        sl = _as_float(result.get("stop_loss"))
    tp1 = _as_float(result.get("tp1"))
    if entry is None or sl is None or tp1 is None:
        return None, "core levels invalid entry=%r sl=%r tp1=%r" % (
            result.get("entry"), result.get("sl") or result.get("stop_loss"), result.get("tp1"))
    direction_display = raw_dir
    ...
    if confidence is None:
        confidence = _as_float(result.get("score"))
    factors = result.get("factors")
    if factors is None:
        factors = result.get("score_breakdown") or result.get("confirmation_factors")
    normalized = {
        "strategy_version": sv,
        "pair": pair,
        "market_type": market_type,
        ...
        "risk": result.get("risk") if result.get("risk") is not None else result.get("rr_to_tp1"),
        "atr": _as_float(result.get("atr")),
        "factors": factors,
        "support": result.get("support"),
        "resistance": result.get("resistance"),
        "setup_explanation": result.get("setup_explanation") or result.get("setup_type"),
...
def emit_alcr_to_dashboard(pair, market_type, result):
    """
    Promote a qualifying ALCR SIGNAL into the main signals table + in-memory
    trade monitor, and post to the PUBLIC Telegram channel.

    Rules:
    - YES: public channel signal + public result cards / public expiry
    - NO: private Telegram chat messages for ALCR
    - YES: news filter (same protection as V2/V3)
    - YES: signal_id, signals table, open_trade, optional AutoBot, push
    """
    if not result or not isinstance(result, dict):
        return False
    status = str(result.get("status") or result.get("classification") or "").upper()
    if status != "SIGNAL" and not result.get("is_signal"):
        return False

    timeframe = result.get("timeframe") or ALCR_SIGNAL_TIMEFRAME
    mapped = dict(result)
    if mapped.get("direction") is None:
        mapped["direction"] = mapped.get("candidate_direction")
    if mapped.get("sl") is None:
        mapped["sl"] = mapped.get("stop_loss")
    if mapped.get("confidence") is None:
        mapped["confidence"] = mapped.get("score")
    if mapped.get("factors") is None:
        mapped["factors"] = mapped.get("score_breakdown")
    if mapped.get("strategy_variant") is None:
        mapped["strategy_variant"] = mapped.get("setup_type")

    strategy_variant = mapped.get("strategy_variant")
    normalized, err = normalize_strategy_signal(
        pair, market_type, timeframe, mapped, "ALCR"
    )
    if err:
        log_error("ALCR dashboard signal rejected (%s %s): %s" % (pair, timeframe, err))
        return False

    direction = normalized["direction"]
    sv = "ALCR"
    raw = normalized.get("_raw_result") or mapped
    if isinstance(raw, dict):
        raw = dict(raw)
        raw["direction"] = direction
        raw["sl"] = normalized["sl"]
        raw["entry"] = normalized["entry"]
        raw["tp1"] = normalized["tp1"]
        raw["tp2"] = normalized.get("tp2")
        raw["tp3"] = normalized.get("tp3")
        raw["confidence"] = normalized.get("confidence")
        raw["strategy_variant"] = strategy_variant

    if is_duplicate_signal(pair, timeframe, direction, strategy_version=sv):
        log_info("ALCR dashboard skip %s (%s): duplicate." % (pair, timeframe))
        return False
    if is_pair_in_cooldown(pair, strategy_version=sv):
        log_info("ALCR dashboard skip %s (%s): pair cooldown." % (pair, timeframe))
        return False
    if has_active_trade_for_pair(pair, strategy_version=sv):
        log_info("ALCR dashboard skip %s (%s): active trade exists." % (pair, timeframe))
        return False

    try:
        if news_engine.is_pair_protected(pair):
            info = news_engine.get_protection_info(pair) or {}
            log_info(
                "ALCR dashboard skip %s (%s): news protection. %s"
                % (pair, timeframe, (info.get("reason") or "high impact window"))
            )
            db_log_activity(
                "signal_blocked_news",
                "NEWS PROTECTION — %s [ALCR] blocked (dashboard path)" % pair,
                strategy_version=sv,
            )
            return False
    except Exception as e:
        log_error("ALCR news protection check failed for %s: %s — blocking" % (pair, e))
        return False

    store_last_signal(pair, timeframe, direction, strategy_version=sv)
    signal_id = get_next_signal_id()

    # PUBLIC channel only (no private chat for ALCR)
    message = format_signal_message(
        pair, timeframe, raw, direction, signal_id, market_type, strategy_version=sv
    )
    sent = send_public_signal(message)
    if not sent:
        log_error(
            "ALCR Signal #%s public Telegram send failed — still writing dashboard row"
            % signal_id
        )

    _record_signal_stats(pair, direction, market_type, timeframe, strategy_version=sv)
    log_info(
        "ALCR Signal #%s sent to public channel + dashboard: %s %s (%s) score=%s entry=%s sl=%s tp1=%s telegram=%s"
        % (
            signal_id, pair, direction, timeframe,
    ...
    try:
        open_auto_trades_for_signal(
            ...
            normalized.get("tp2"), normalized.get("tp3"),
            signal_id, strategy_version=sv,
        )
    except Exception as e:
        log_error("ALCR AutoBot open failed for Signal #%s (non-fatal): %s" % (signal_id, e))
    db_log_activity(
        "signal_sent",
        "ALCR Signal #%s public channel + dashboard: %s %s (%s) score %s"
        % (signal_id, pair, direction, timeframe, normalized.get("confidence")),
        strategy_version=sv,
    )
    try:
        send_push_to_all(
            "new_signal_alerts",
            "SmartFX ALCR Signal",
            "%s — %s [ALCR]\nScore: %s" % (pair, direction, normalized.get("confidence")),
            url="dashboard.html?signal=%s" % signal_id,
        )
    except Exception:
        pass
...
def emit_signal_from_result(pair, market_type, timeframe, result, strategy_version, strategy_variant=None):
    ...
    # High-Impact news protection (NEW signals only). Strategy gates unchanged.
    try:
        if news_engine.is_pair_protected(pair):
            info = news_engine.get_protection_info(pair) or {}
            reason = info.get("reason") or (
                "NEWS PROTECTION — %s blocked (High Impact event window active)" % pair
            )
            log_info(
                "Skipping %s signal for %s (%s): news protection. %s"
                % (sv, pair, timeframe, reason.replace("\n", " | "))
            )
            db_log_activity(
                "signal_blocked_news",
                "NEWS PROTECTION — %s [%s] blocked: %s — %s"
                % (
                    pair,
                    sv,
                    info.get("currency", "?"),
                    info.get("event", "High Impact event"),
                ),
                strategy_version=sv,
            )
            return False
    except Exception as e:
        # Fail-safe: never silently bypass protection on engine errors
        log_error("News protection check failed for %s: %s — blocking signal" % (pair, e))
        db_log_activity(
            "signal_blocked_news",
            "NEWS PROTECTION — %s [%s] blocked: protection check error (fail-safe)"
            % (pair, sv),
            strategy_version=sv,
        )
        return False
...
def analyze_pair_v3(pair, market_type):
    ...
    Observability: strategy_scan_log + one activity event per evaluation.
    """
    ...
    try:
        ...
        if not bundle:
            db_insert_strategy_scan_log(
                pair, market_type, "V3", V3_SIGNAL_TIMEFRAME, None, False,
                "NO_DATA", reason="insufficient_candles",
            )
            _log_scan_activity("V3", pair, "NO_DATA", strategy_version="V3")
            db_update_pair_status(
                pair, market_type, None, "WATCHING", None, None, strategy_version="V3"
            )
            return
        c4, c1, c15, c5 = bundle
        gate_results, failed_gate, reason, bias = _diagnose_v3_pipeline(c4, c1, c15, c5)

        # Existing strategy call — unchanged
        try:
            result = smc_analysis_v3.analyze_v3(c4, c1, c15, c5)
        except Exception as e:
            log_error("V3 analyze_v3 error for %s: %s" % (pair, e))
            db_insert_strategy_scan_log(
                pair, market_type, "V3", V3_SIGNAL_TIMEFRAME, bias, bool(bias in ("BUY", "SELL")),
                "ERROR", failed_gate=failed_gate, reason=str(e), gate_results=gate_results,
            )
            _log_scan_activity("V3", pair, "ERROR", candidate_direction=bias, strategy_version="V3")
            trend = bias if bias in ("BUY", "SELL") else None
            db_update_pair_status(
        ...
        candidate_detected = bias in ("BUY", "SELL")
        if result is None:
            db_insert_strategy_scan_log(
                pair, market_type, "V3", V3_SIGNAL_TIMEFRAME, bias, candidate_detected,
                "NO_SIGNAL", failed_gate=failed_gate, reason=reason, gate_results=gate_results,
            )
            _log_scan_activity(
                "V3", pair, "NO_SIGNAL",
                candidate_direction=bias, failed_gate=failed_gate, strategy_version="V3",
            )
            trend = bias if bias in ("BUY", "SELL") else None
            db_update_pair_status(
                pair, market_type, trend, "WATCHING", None, None, strategy_version="V3"
        ...
        # Signal path
        if not gate_results or (failed_gate is None and reason == "all_required_gates_passed"):
            gate_results = [{"gate": g, "result": "PASS"} for g in V3_GATE_ORDER]
        ...
        if "BUY" in str(direction).upper():
            ...
            d = bias if bias in ("BUY", "SELL") else None
        db_insert_strategy_scan_log(
            pair, market_type, "V3", V3_SIGNAL_TIMEFRAME, d, True,
            "SIGNAL", reason="all_required_gates_passed", gate_results=gate_results,
        )
        _log_scan_activity("V3", pair, "SIGNAL", candidate_direction=d, strategy_version="V3")
        emit_signal_from_result(
        ...
        try:
            ...
            _log_scan_activity("V3", pair, "ERROR", strategy_version="V3")
        except Exception:
            pass
...
def safe_analyze(pair, market_type):
    ...
    # --- ALCR RESEARCH ENGINE ---
    try:
        run_alcr_analysis_for_pair(pair, market_type)
    except Exception as e:
        log_error(f"ALCR engine error analyzing {pair} (isolated): {e}")

    # --- SMC ML RESEARCH ENGINE (Supabase only, no Telegram) ---
    try:
        run_smc_ml_analysis_for_pair(pair, market_type)
    except Exception as e:
        log_error(f"SMC ML engine error analyzing {pair} (isolated): {e}")
...
def trade_monitor_loop():
    while True:
        try:
            ...
            monitor_alcr_research_trades()
            monitor_smc_ml_research_trades()
        except Exception as e:
...
def start_background_threads():
    ...
    # Wire news engine to existing Telegram + activity_log helpers (no circular imports)
    news_engine.configure_callbacks(
        send_public_message=send_public_signal,
        send_public_photo=send_public_photo,
...
@app.route("/health")
def health_route():
    """Web process + background loop health. Prefer additive fields for dashboard."""
    now = time.time()
    # Stale thresholds (seconds) — analysis typically cycles faster; monitor similar
    ANALYSIS_STALE = 900   # 15 min
    MONITOR_STALE = 600    # 10 min
    SUMMARY_STALE = 90000  # daily loop can be quiet for long stretches

    with state_lock:
        v2 = dict(_global_bucket("V2"))
        v3 = dict(_global_bucket("V3"))
        active_count = len(active_trades)
        hb = dict(loop_heartbeats)
        err_count = _ops_errors

    def _age(name):
        ts = hb.get(name)
        if not ts:
            return None
        return round(now - float(ts), 1)

    def _ok(name, stale_after):
        age = _age(name)
        if age is None:
            return False
        return age <= stale_after

    analysis_ok = _ok("analysis", ANALYSIS_STALE)
    monitor_ok = _ok("trade_monitor", MONITOR_STALE)
    summary_ok = _ok("daily_summary", SUMMARY_STALE)

    # Overall: web is up; "healthy" only if analysis + trade_monitor heartbeats are fresh
    if analysis_ok and monitor_ok:
        overall = "healthy"
    elif analysis_ok or monitor_ok:
        overall = "degraded"
    else:
        overall = "unhealthy"

    # Optional last_scan_at from bot_status (best-effort, non-fatal)
    last_scan_at = None
    try:
        if DATABASE_URL:
            conn = get_db_connection()
            with conn:
                with conn.cursor() as cur:
                    cur.execute("SELECT last_scan_at FROM bot_status WHERE id = 1")
                    row = cur.fetchone()
                    if row and row[0] is not None:
                        last_scan_at = row[0].isoformat() if hasattr(row[0], "isoformat") else str(row[0])
            conn.close()
    except Exception:
        pass
    ...
    return jsonify({
        ...
        "health": overall,
        "web_alive": True,
        "analysis_loop_ok": analysis_ok,
        "trade_monitor_ok": monitor_ok,
        "daily_summary_ok": summary_ok,
        "heartbeats": {
            "analysis": hb.get("analysis"),
            "trade_monitor": hb.get("trade_monitor"),
            "daily_summary": hb.get("daily_summary"),
            "analysis_age_sec": _age("analysis"),
            "trade_monitor_age_sec": _age("monitor") if False else _age("trade_monitor"),
            "daily_summary_age_sec": _age("daily_summary"),
        },
