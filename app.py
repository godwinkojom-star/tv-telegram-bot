...
def open_auto_trades_for_signal(pair, market_type, direction, entry, sl, tp1, tp2, tp3, signal_id):
    ...
    SL/TP calculation of its own. Position size is derived purely from
    each user's own real balance and their chosen risk%, so two users
    with different balances get different position sizes for the exact
    same signal, and each user's risk in dollars at the real SL is
    always exactly balance * risk_pct - the whole point of sizing this
    way instead of a fixed dollar stake.
    """
    if not DATABASE_URL:
        return
                    ...
                    cur.execute("""
                        INSERT INTO auto_trades
                            (user_id, signal_id, pair, market_type, direction,
                             entry, sl, tp1, tp2, tp3, risk_pct, risk_amount, position_size)
