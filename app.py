...
#          ALCR uses the shared get_candles()/OHLC cache, writes only to
#          alcr_research_log, never enters the live signal/Telegram/AutoBot
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                """, (
                    ...
                    result.get("expires_at"), result.get("session")
                ))
        conn.close()
    except Exception as e:
...
def analyze_pair_alcr(pair, market_type):
    """Run ALCR research analysis only; isolated from V2/V3/live execution."""
