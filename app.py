...
KRAKEN_INTERVAL_MAP = {"4h": 240, "1h": 60, "15m": 15, "5m": 5}
TWELVEDATA_INTERVAL_MAP = {"4h": "4h", "1h": "1h", "15m": "15min", "5m": "5min"}
...
TIMEFRAME_MINUTES = {"1h": 60, "15m": 15, "5m": 5}
...
def db_insert_signal(pair, market_type, timeframe, direction, result, signal_id, strategy_version="V2"):
    """
    Insert a signal row. strategy_version mandatory for NEW rows (F8): "V2" or "V3".
    Refuses invalid/NULL identity so new rows never get strategy_version NULL.
    """
...
def compute_lot_sizing(pair, entry, sl, balance, risk_pct):
    ...
    If the risk-safe lot is below broker min_volume (0.01), use
    min_volume instead of skipping — provided the account balance can
    cover the dollar risk of that minimum lot (balance/margin safety).
    Still respects volume_step and max_volume. As balance grows, larger
    risk-justified lots are used when calculated above the minimum.
    """
    ...
    if lot < min_volume:
        ...
        if min_lot_risk > balance:
            # Not enough balance/margin for even the broker minimum.
            return None
        lot = min_volume
...
def get_candles(pair, timeframe, market_type, use_cache=True):
    ...
    if candles:
        with state_lock:
            ...
            if market_type == "forex" and timeframe in ("1h", "15m", "5m"):
...
def format_signal_message(pair, timeframe, result, trend, signal_id, market_type, strategy_version="V2"):
    """Stage F9: existing signal Telegram format + Strategy line. strategy_version V2|V3 required."""
    sv = _normalize_strategy_version(strategy_version)
    if sv not in ("V2", "V3"):
        raise ValueError("format_signal_message: invalid strategy_version=%r" % (strategy_version,))

    direction_word = "BUY" if "BUY" in str(result.get("direction", "")) else "SELL"
    direction_emoji = "🟢" if direction_word == "BUY" else "🔴"
    trend_text = "Bullish" if trend == "BUY" else ("Bearish" if trend == "SELL" else str(trend or "N/A"))
    market_hashtag = "Crypto" if market_type == "crypto" else "Forex"
    pair_hashtag = pair.replace("/", "")

    extra = ""
    if sv == "V3" and result.get("setup_explanation"):
        extra = "\n📝 Setup: %s\n" % (result["setup_explanation"],)
    ...
    return (
        ...
        f"📈 Resistance: `{result.get('resistance')}`\n\n"
        f"📊 Signal Strength: {result.get('confidence')}%\n"
        f"⚠️ Risk: {result.get('risk')}\n"
        f"📈 Trend: 4H {trend_text}\n"
        f"{extra}"
        f"\n🆔 Signal #{signal_id}\n\n"
        f"{RISK_DISCLAIMER}\n\n"
        f"#{pair_hashtag} #{market_hashtag} #Strategy{sv}"
    )
...
def is_pair_in_cooldown(pair):
    """
    Pair-wide cooldown (V2 behavior preserved).
    POLICY DEFERRED: when strategy V3 is ON, V2 and V3 still share this pair
    cooldown and will block each other. Per-strategy cooldown is a later decision.
    """
    now = time.time()
    with state_lock:
...
def mark_pair_signal_time(pair):
    """Pair-wide last-signal timestamp (unchanged)."""
    with state_lock:
...
def has_active_trade_for_pair(pair):
    """
    Pair-wide active-risk gate (V2 safety preserved).
    POLICY DEFERRED: still blocks V2 and V3 alike on this pair while a
    pre-TP1 trade is open. Does not auto-allow two simultaneous strategy trades.
    """
    with state_lock:
...
def resume_open_trades_from_db():
    ...
    for (signal_id, pair, market_type, timeframe, direction,
         entry, sl, tp1, tp2, tp3, status, final_level, sent_at, strategy_version) in rows:

        tp1_hit = status == "WIN"
        tp2_hit = tp1_hit and final_level == "TP2"

        # Preserve identity when present; do not invent for historical NULL
...
def open_trade(pair, timeframe, market_type, direction, result, signal_id, strategy_version="V2"):
...
def analyze_pair(pair, market_type):
    ...
    for timeframe in ENTRY_TIMEFRAMES:
        ...
        if not is_strategy_enabled("V2"):
            log_info(f"Skipping V2 signal for {pair} ({timeframe}): strategy V2 is OFF.")
            continue

        if is_duplicate_signal(pair, timeframe, direction, strategy_version="V2"):
            continue
        ...
        store_last_signal(pair, timeframe, direction, strategy_version="V2")

        signal_id = get_next_signal_id()
        message = format_signal_message(pair, timeframe, result, trend, signal_id, market_type, strategy_version="V2")
        ...
        if sent:
            ...
            open_trade(pair, timeframe, market_type, direction, result, signal_id, strategy_version="V2")
            db_insert_signal(pair, market_type, timeframe, direction, result, signal_id, strategy_version="V2")
            open_auto_trades_for_signal(
                pair, market_type, direction,
                result["entry"], result["sl"], result["tp1"], result["tp2"], result["tp3"],
                signal_id, strategy_version="V2",
            )
            db_log_activity(
                "signal_sent",
                f"Signal #{signal_id} [V2] sent: {pair} {direction} ({timeframe}) - confidence {result['confidence']}%",
            )
            send_push_to_all(
                "new_signal_alerts",
                "SmartFX Signal 🚨",
...
def run_v3_analysis(pair, market_type):
    """Thin adapter: candles → smc_analysis_v3.analyze_v3. No gate changes. Returns dict or None. Exceptions swallowed."""
    bundle = fetch_v3_candle_bundle(pair, market_type)
    if bundle is None:
        return None
    candles_4h, candles_1h, candles_15m, candles_5m = bundle
    try:
        return smc_analysis_v3.analyze_v3(candles_4h, candles_1h, candles_15m, candles_5m)
    except Exception as e:
...
def emit_signal_from_result(pair, market_type, timeframe, result, strategy_version):
    ...
    log_info(
        "Signal #%s sent [%s]: %s %s (%s) confidence=%s entry=%s sl=%s tp1=%s"
        % (signal_id, sv, pair, direction, timeframe,
           normalized.get("confidence"), normalized.get("entry"),
           normalized.get("sl"), normalized.get("tp1"))
    )
    mark_pair_signal_time(pair)
    open_trade(pair, timeframe, market_type, direction, raw, signal_id, strategy_version=sv)
    db_insert_signal(pair, market_type, timeframe, direction, raw, signal_id, strategy_version=sv)
    open_auto_trades_for_signal(
        pair, market_type, direction,
    ...
    db_log_activity(
        "signal_sent",
        "Signal #%s [%s] sent: %s %s (%s) - confidence %s%%"
        % (signal_id, sv, pair, direction, timeframe, normalized.get("confidence")),
    )
    send_push_to_all(
        "new_signal_alerts",
        "SmartFX Signal",
...
def analyze_pair_v3(pair, market_type):
    ...
    if not is_strategy_enabled("V3"):
        return
    try:
        result = run_v3_analysis(pair, market_type)
        if result is None:
            return
        # Shared emission path with strategy_version="V3"
...
def safe_analyze(pair, market_type):
    """
    Stage F7: ONE scanner path per pair — V2 first, then V3.
    Independent enable checks via is_strategy_enabled (F5).
    Separate try blocks so a V3 failure never aborts V2 (and vice versa).
    No second loop, no second app, no pair ownership.
    """
    # --- V2 ENGINE (production default ON) ---
    if is_strategy_enabled("V2"):
        try:
            analyze_pair(pair, market_type)
        except Exception as e:
            log_error(f"V2 engine error analyzing {pair}: {e}")
    # else: V2 OFF — skip NEW V2 analysis for this pair this cycle
    # (open trade monitoring continues in trade_monitor_loop)

    # --- V3 ENGINE (default OFF; same pairs, independent) ---
    try:
...
def build_morning_report():
    ...
    return (
        ...
        "✅ Bot Status: ONLINE\n\n"
        f"🏷 Strategies: V2={'ON' if is_strategy_enabled('V2') else 'OFF'} · V3={'ON' if is_strategy_enabled('V3') else 'OFF'}\n\n"
...
def start_background_threads():
    ...
    log_info(
        ...
        f"({BOT_NAME} v{VERSION} / smc_analysis v{smc_analysis.SMC_VERSION} / "
    ...
    db_log_activity(
        "bot_restart",
        f"Bot started ({BOT_NAME} v{VERSION} / smc_analysis v{smc_analysis.SMC_VERSION} / "
        f"v3 {getattr(smc_analysis_v3, 'V3_VERSION', '?')} "
        f"V2={'ON' if is_strategy_enabled('V2') else 'OFF'} "
        f"V3={'ON' if is_strategy_enabled('V3') else 'OFF'})",
    )
