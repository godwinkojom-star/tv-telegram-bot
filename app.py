...
def monitor_trades():
    ...
    for trade_id, trade in trades_snapshot:
        ...
        if not tp1_hit:
            ...
            if outcome == "WIN":
                with state_lock:
                    if trade_id in active_trades:
                        ...
                        variant = active_trades[trade_id].get("strategy_variant")
                        if variant == "GOLD_SCALPING" or (
                            active_trades[trade_id].get("strategy_version") == "V2"
                            and active_trades[trade_id].get("pair") == "XAU/USD"
                        ):
                            # Gold V2: 50% partial conceptual + protected-profit (not exact entry BE)
                            entry = float(active_trades[trade_id]["entry"])
                            tp1 = float(active_trades[trade_id]["tp1"])
                            # Protected stop between entry and TP1 (locked profit, not exact BE)
                            active_trades[trade_id]["protected_stop"] = round((entry + tp1) / 2.0, 6)
                            active_trades[trade_id]["management_stage"] = "TP1_PARTIAL"
                            active_trades[trade_id]["gold_partial_closed"] = True
                            active_trades[trade_id]["outcome_type"] = "Partial"
            ...
            is_gold = (
                trade.get("strategy_variant") == "GOLD_SCALPING"
                or (trade.get("strategy_version") == "V2" and trade.get("pair") == "XAU/USD")
            )
            if is_gold and trade.get("protected_stop") is not None:
                # Gold V2: exit remaining on protected-profit stop (not exact entry BE)
                prot = float(trade["protected_stop"])
                protected_hit = (
                ...
                if protected_hit:
                    db_update_signal_status(
                        trade.get("signal_id"), "WIN", final_level="PROTECTED", closed=True
                    )
                    close_auto_trades_for_signal(
                        trade.get("signal_id"), "WIN", final_level="TP1"
                    )
                    with state_lock:
                        if trade_id in active_trades:
                            active_trades[trade_id]["outcome_type"] = "Protected Win"
                            active_trades[trade_id]["management_stage"] = "CLOSED_PROTECTED"
                        active_trades.pop(trade_id, None)
                    continue
                breakeven_hit = False
            else:
