...
def analyze_pair_v2(pair, market_type):
    ...
    try:
        ...
        emit_signal_from_result(
            pair, market_type, V2_SIGNAL_TIMEFRAME, mapped, strategy_version="V2",
        )
        d = mapped["direction"]
        db_update_pair_status(
...
def analyze_pair(pair, market_type):
    ...
    New Gold V2 (smc_analysis_v2.py, 15M→5M→1M) is NOT connected yet.
    V3 continues via analyze_pair_v3 / safe_analyze.
    Historical V2 signals/trades in the database are left intact.
    """
    log_info(
        "Old V2 runtime skipped for %s (%s): disconnected; waiting for new V2 wiring."
        % (pair, market_type)
    )
    return
...
def fetch_v3_candle_bundle(pair, market_type):
    ...
    """
    if pair == "XAU/USD" or str(pair or "").upper().replace(" ", "") in ("XAU/USD", "XAUUSD"):
        return None
    series = {}
...
def analyze_pair_v3(pair, market_type):
    ...
    if not is_strategy_enabled("V3"):
        return
    # Gold/XAU is exclusive to new Gold V2 — never enter V3 pipeline
    if pair == "XAU/USD" or str(pair or "").upper().replace(" ", "") in ("XAU/USD", "XAUUSD"):
        return
    try:
