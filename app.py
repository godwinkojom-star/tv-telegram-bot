...
def get_auto_trading_users():
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                # Return user_id + risk only. Strategy-specific balance is
                # loaded via get_portfolio_balance_for_strategy(balance_v2/v3).
                cur.execute("""
                    SELECT us.user_id, us.auto_trading_risk_pct, 0
...
def analyze_pair(pair, market_type):
    ...
    for timeframe in ENTRY_TIMEFRAMES:
        ...
        if is_pair_in_cooldown(pair, strategy_version="V2"):
            log_info(f"Skipping signal for {pair} ({timeframe}): pair cooldown active.")
            continue

        if has_active_trade_for_pair(pair, strategy_version="V2"):
        ...
        if sent:
            _record_signal_stats(pair, direction, market_type, timeframe, strategy_version="V2")
            ...
            mark_pair_signal_time(pair, strategy_version="V2")
    ...
    db_update_pair_status(pair, market_type, trend, scan_status, scan_confidence, scan_direction, strategy_version="V2")
