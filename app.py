...
def compute_lot_sizing(pair, entry, sl, balance, risk_pct):
    ...
    only — never invents levels:
      ...
      lot = floor(raw_lot to volume_step), clamped to [min_volume, max_volume]

    If the risk-safe lot is below the broker min_volume, return None so
    the caller skips that user rather than forcing the minimum (which
    would exceed the configured risk %). As balance and/or risk % grow,
    the same formula naturally produces larger valid lots when justified.
    """
    ...
    # Never force the broker minimum if it would exceed configured risk.
    if lot < min_volume:
        return None

    if lot > max_volume:
        lot = max_volume
        # After clamping to max, re-check risk ceiling (max lot can still
        # be too large for a very tight SL on a small account — rare).
        if dollar_risk_for_lot(lot) > risk_ceiling + 1e-9:
            # Step down from max until under ceiling or below min.
            while lot >= min_volume and dollar_risk_for_lot(lot) > risk_ceiling:
                lot = round(lot - volume_step, 8)
            if lot < min_volume:
                return None
...
def open_auto_trades_for_signal(pair, market_type, direction, entry, sl, tp1, tp2, tp3, signal_id):
    ...
    from SmartFX's Entry/SL, floored to volume_step and validated against
    min/max volume. Crypto pairs keep the original balance*risk% /
            ...
            with conn.cursor() as cur:
                for user_id, risk_pct, balance in users:
                    ...
                    if lot_sizing:
                        ...
                        # Forex pair, but risk-safe lot is below broker
                        # min_volume (or invalid) for this balance/SL —
                        # skip rather than force the minimum and exceed risk.
                        continue
                    else:
