...
def load_mascot(outcome):
    ...
    Returns an RGBA PIL Image, or None if the file is missing or
    unreadable - callers must treat None as "skip the mascot, render
    the card exactly as before", never as an error to surface to the
    user. Results are cached in memory since the same two files are
    reused for every card - avoids re-reading disk on every trade.
    """
    ...
    try:
        ...
        log_error(f"Could not load mascot asset '{filename}': {e}")
...
def generate_result_card(trade, outcome, final_tp_label=None):
    ...
    # ---- Mascot (visual only) ----
    # Reuses the exact same `outcome` value already used for every
    # other decision in this function - no second win/loss system.
    # Loaded from the asset files, not embedded in source, so the
    # artwork can be swapped later with zero code changes. Any
    # problem loading it (missing file, bad format) is caught and
    # simply skipped - the card still renders exactly as it did
    ...
    try:
        ...
        if mascot is not None:
            ...
            margin = 20
            pos_x = width - mascot_size - margin
            pos_y = height - mascot_size - margin
            img.paste(mascot, (pos_x, pos_y), mascot)
    except Exception as e:
