    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                """)
        conn.close()
        log_info("Database ready: 'signals' table confirmed/created.")
    except Exception as e:
...
def db_update_signal_status(signal_id, status, final_level=None, closed=False):
    if not DATABASE_URL:
        return
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE signal_id = %s
                """, (status, final_level, closed, datetime.utcnow(), signal_id))
        conn.close()
    except Exception as e:
...
def check_forex_candles_for_hit(trade):
    ...
    exactly which candle and price caused the result - this is what
    lets us prove/disprove a bug instead of guessing from timestamps.
    """
...
def close_trade(trade_id, trade, outcome, keep_tracking=False, trigger_info=None):
    ...
    if outcome == "LOSS":
        ...
        db_update_signal_status(signal_id, "LOSS", closed=True)
    else:
        # TP1 secured - a WIN either way, but tracking continues for
        # TP2/TP3, so the database row isn't closed yet, just updated.
        db_update_signal_status(signal_id, "WIN", final_level="TP1", closed=not keep_tracking)
    ...
    for trade_id, trade in trades_snapshot:
        ...
        if not tp1_hit:
            ...
            if breakeven_hit:
                ...
                send_result_card(trade, "WIN", final_tp_label=final_label)
        ...
        if age_seconds > expiry_seconds:
            if tp1_hit:
                ...
                # TP2/TP3, and send the card now showing whichever
                # level actually ended up being the final one reached.
                final_label = "TP2" if trade.get("tp2_hit") else "TP1"
                send_result_card(trade, "WIN", final_tp_label=final_label)
                db_update_signal_status(trade.get("signal_id"), "WIN", final_level=final_label, closed=True)
                with state_lock:
