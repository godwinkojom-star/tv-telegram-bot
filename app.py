...
#          open_trade monitoring, optional AutoBot. NO Telegram public or
#          private messages / result cards for ALCR. Research log unchanged.
#
...
def send_result_card(trade, outcome, final_tp_label=None):
    # ALCR is dashboard-only: never post result cards to Telegram
    if _normalize_strategy_version((trade or {}).get("strategy_version")) == "ALCR":
        return False
    try:
...
def close_trade(trade_id, trade, outcome, keep_tracking=False, trigger_info=None):
    ...
    # ALCR is dashboard-only: never send win/loss to Telegram
    if not _is_alcr:
...
def expire_trade(trade_id, trade):
    ...
    # ALCR is dashboard-only: no Telegram expiry messages
    if not _is_alcr:
        send_private_message(
...
def emit_alcr_to_dashboard(pair, market_type, result):
    ...
    trade monitor so the dashboard can show it like V2/V3.

    Hard rules (user request):
    - NO Telegram public channel message
    - NO Telegram private win/loss/expired messages
    - NO result cards to Telegram
    - YES: signal_id, signals table, open_trade monitor, optional AutoBot,
           activity_log, browser push (dashboard)
    """
    if not result or not isinstance(result, dict):
        return False
    ...
    log_info(
        "ALCR Signal #%s logged to dashboard (no Telegram): %s %s (%s) score=%s entry=%s sl=%s tp1=%s"
        % (
    ...
    db_log_activity(
        "signal_sent",
        "ALCR Signal #%s logged to dashboard (no Telegram): %s %s (%s) score %s"
