...
def analyze_candles(candles, trend_4h=None):
    ...
    if (
        ...
        # Hard requirement: at least one of CHOCH, liquidity sweep, FVG,
        # or a Dragonfly Doji must confirm this direction. Without this,
        # BOS alone can fire the instant a level breaks, with no proof
        # the move actually holds - a classic fakeout setup. Any ONE of
        # these four confirming is enough - this keeps signal frequency
        # reasonable while still requiring real confirmation.
        confirmed = (
            choch == "BUY"
        ...
        if confidence >= 80 and confirmed:
            ...
            # If pulling TP1 back from a nearby resistance wall left it
            # too close to entry relative to the stop loss (a broken
            # risk-reward ratio), skip the signal entirely rather than
            # send a trade that risks far more than it can gain.
            risk = entry - sl
            reward = tp1 - entry
