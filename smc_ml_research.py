...
def analyze_1h(
    ...
    Looks for a BOS / CHoCH that occurred on any of the last `signal_lookback`
    bars (default 5). This way the research scanner can still pick up a
    recent structure break even if the exact break bar is not the current one.
    """
    ...
    # Walk full history to maintain structure state; record events in the
    # recent lookback window so we do not require the break to be exactly
    # on the final bar.
    last_sh = None
    last_sl = None
    structure = 0  # 1 bullish, -1 bearish

    recent_events = []  # list of (index, "BOS"|"CHOCH", "BUY"|"SELL")
    n = len(df)
    ...
    for i in range(n):
        ...
        if last_sh is not None and close > last_sh:
            event_type = "BOS" if structure == 1 else "CHOCH"
            if i >= lookback_start:
                recent_events.append((i, event_type, "BUY"))
            structure = 1
            last_sh = None
        ...
        if last_sl is not None and close < last_sl:
            event_type = "BOS" if structure == -1 else "CHOCH"
            if i >= lookback_start:
                recent_events.append((i, event_type, "SELL"))
            structure = -1
            last_sl = None
    ...
    # Use the most recent structure event inside the lookback window
    _, structure_type, direction = recent_events[-1]
