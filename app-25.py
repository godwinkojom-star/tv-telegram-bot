    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                """)
        conn.close()
        log_info("Database ready: 'signals' and 'bot_status' tables confirmed/created.")
    except Exception as e:
...
def db_update_bot_status():
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute("""
                    INSERT INTO bot_status (id, last_scan_at, bot_version)
                    VALUES (1, %s, %s)
...
def check_forex_candles_for_hit(trade):
    ...
    for i, c in enumerate(candles):
        ...
        if hit_sl or hit_tp1:
            ...
            ambiguity_tag = "AMBIGUOUS_SAME_CANDLE (both SL and TP1 in range - defaulted to LOSS) | " if both_hit_same_candle else ""
            ...
            if both_hit_same_candle:
                log_error(
                    ...
                    f"C={c['close']}) - defaulted to LOSS since intrabar order is unknown."
                )
