...
def fetch_news_events():
    ...
    if not FMP_API_KEY:
        return {"events": [], "provider_configured": False, "request_ok": False,
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            # request_ok=False is the key fix here - a failed HTTP call
            # (402 restricted-endpoint, 401 bad key, etc.) must never
            # be reported the same way as a real, successful call that
            # simply found nothing relevant this week.
            return {"events": [], "provider_configured": True, "request_ok": False,
        ...
        return {"events": relevant_events, "provider_configured": True, "request_ok": True,
...
def run_cycle():
    ...
    try:
        ...
        if not result["provider_configured"]:
            status = "HEALTHY — Provider not configured — Engine idle"
        elif not result["request_ok"]:
            # This is the actual fix: a failed HTTP request (402
            # restricted-endpoint, 401 bad key, timeout, etc.) now
            # reports as DEGRADED, never as "HEALTHY — provider
            # active" - the earlier version conflated "0 events" with
            # "the call actually failed," which is exactly what made a
            # real 402 error look like everything was fine.
            status = f"DEGRADED — FMP request failing — {result['reason']}"
        elif result["events"]:
